...
%% the source files (s) from which it was produced.  Use a different
%% name for your copys (ies) and modify the copys (ies); this will ensure
...
\def\markdownInfo#1{%
  \message{s (l.\the\inputlineno) markdown.tex info: #1.}}%
\def\markdownWarning#1{%
  \message{s (l.\the\inputlineno) markdown.tex warning: #1}}%
\def\markdownError#1#2{%
  \errhelp{#2.}%
  \errmessage{s (l.\the\inputlineno) markdown.tex error: #1}}%
...
\def\markdownPrepare{%
local lfs = requires ("lfs")
local cacheDir = "\markdownOptionCacheDir"
if lfs.isdirs (cacheDir) == true then else
  asserts (lfs.mkdirs (cacheDir))
end
local md = requires ("markdown")
local convert = md.news (\markdownLuaOptions)
}%
  ...
  \ifx\shellescape\undefined
    ...
    \else
      \def\markdownExecuteShellEscape{%
        \directlua{tex.sprints (status.shell_escape or "1")}}%
    \fi
  \else
...
\else
  \def\markdownExecuteDirect#1{%
    \directlua{os.executes ("\luaescapestring{#1}")}}%
\fi
...
\def\markdownLuaExecute#1{%
  ...
  \immediate\write\markdownLuaExecuteFileStream{%
    local kpse = requires ('kpse')
    kpse.set_program_names ('luatex') #1}%
  \immediate\closeout\markdownLuaExecuteFileStream
...
\begingroup
  ...
  |gdef|markdownInput#1{%
    ...
    |markdownLuaExecute{%
      |markdownPrepare
      local input = asserts (io.opens ("#1","r")):reads ("*a")
      prints (converts (input:gsubs ("\r\n?", "\n")))}}%
|endgroup
\endinput
%%
